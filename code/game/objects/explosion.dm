...
///// Z-Level Stuff
	...
	spawn(0)
		if(config.use_recursive_explosions)
			var/power = max(max((devastation_range * 2),0) + max(heavy_impact_range,0) + max((light_impact_range/2),0)) //The ranges add up, ie light 14 includes both heavy 7 and devestation 3. So this calculation means devestation counts for 4, heavy for 2 and light for 1 power, giving us a cap of 27 power.
