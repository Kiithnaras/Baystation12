...
//Large Queens (empresses) can open regular doors faster and even bypass some of the measures, such as locked and welded doors.
	...
	if(src.density && isAlien && !aforce)
		if(istype(src, /obj/machinery/door/airlock)) //Airlock-specific conditions
			...
			if(operating)
				to_chat(user, "\green You cannot force an airlock that is in motion!") //Well duh!
				return
			...
			if(AF.locked) //Behavior for bolted airlocks - They can't be opened period. Gotta melt them suckers! We also don't need to bother checking for power here.
				...
				user.visible_message("<span class='notice'>[user] digs its claws in and starts to pry \the door open!</span>",\
					...
					"<span class='notice'>You hear a loud metal thunk!</span>")
				if(do_after(user,20))
					user.visible_message("<span class='notice'>[user] struggles ineffectively against \the door!</span>",\
						"\green You struggle ineffectively against the door.",\
						"You hear a faint metal scratching.")
					aforce = 0
					return
				else
