...
/obj/item/weapon/mining_scanner/attack_self(mob/user as mob)
	...
	for(var/turf/simulated/T in range(2, get_turf(user)))
		...
		for(var/metal in T.resources)
			...
			switch(metal)
				if(MATERIAL_IRON, MATERIAL_ALUMINIUM, MATERIAL_BAUXITE, MATERIAL_HEMATITE, MATERIAL_PYRITE)
					ore_type = "surface minerals"
				if(MATERIAL_GOLD, MATERIAL_SILVER, MATERIAL_PLATINUM, MATERIAL_DIAMOND, MATERIAL_OSMIUM)
