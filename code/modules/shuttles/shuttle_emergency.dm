...
/datum/shuttle/autodock/ferry/emergency/New()
	...
	if (emergency_controller.shuttle)
		...
		return
	move_time = emergency_controller.evac_transit_delay/10
...
/datum/shuttle/autodock/ferry/emergency/arrived()
	...
	if (!emergency_controller.has_evacuated())
		...
		emergency_controller.finish_evacuation()
...
/datum/shuttle/autodock/ferry/emergency/long_jump(var/destination, var/interim, var/travel_time)
	..(destination, interim, emergency_controller.get_long_jump_time())
*/
/datum/shuttle/autodock/ferry/emergency/shuttle_moved()
	if(next_location != waypoint_station && current_location == waypoint_station)
	...
	else if(current_location == waypoint_offsite && emergency_controller.has_evacuated())
...
/obj/machinery/computer/shuttle_control/emergency
	name = "departure shuttle control console"
	shuttle_tag = "Departure"
	req_one_access = list(access_bridge,access_lawyer,access_heads)
	var/debug = 0
...
/obj/machinery/computer/shuttle_control/emergency/emag_act(var/remaining_charges, var/mob/user)
	if (!emagged)
		...
		return 1
*/
/obj/machinery/computer/shuttle_control/emergency/attackby(obj/item/weapon/card/W as obj, mob/user as mob)
	read_authorization(W)
	..()

/obj/machinery/computer/shuttle_control/emergency/get_ui_data(var/datum/shuttle/autodock/ferry/shuttle)
	var/shuttle_state
	...
	switch (shuttle.process_state)
		if(IDLE_STATE)
			if (shuttle.in_use)
				shuttle_status = "Busy."
			else if (shuttle.current_location == shuttle.waypoint_offsite)
...
/obj/machinery/computer/shuttle_control/emergency/handle_topic_href(var/datum/shuttle/autodock/ferry/shuttle, var/list/href_list, var/user)
	if(!istype(shuttle))
